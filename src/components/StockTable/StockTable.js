            ...
            <div>
              <Row gutter={64}>
                <Col span={8}>
                  ...
                  <InputNumber value={(skuStocks[`${selectWarehouseId}`] || {}).store_quantity} formatter={value => ( value ? Math.floor(value/selecStockUnitNum).toString() : value )} parser={value =>(Number(value)*selecStockUnitNum).toString()} onChange={this.handleStockInput.bind(null,-1,-1)}  placeholder='请输入' min={0} precision={2} style={{width:'100%'}}/>
                </Col>
              </Row>
            </div>
          ) : (
                    ...
                    selectColors.map( item => {
                      return (
                        <Col key={item.id} span={8}>
                          ...
                          <InputNumber value={(skuStocks[`${selectWarehouseId}_${item.id}`] || {} ).store_quantity} formatter={value => ( value ? Math.floor(value/selecStockUnitNum).toString() : value )} parser={value =>(Number(value)*selecStockUnitNum).toString()} onChange={this.handleStockInput.bind(null,item.id,-1)} placeholder='请输入' precision={2} min={0}  style={{width:'100%'}}/>
                        </Col>
                      )
                    })
                  }
