...
export default {
  ...
  effects: {
    ...
    *createSingle({ payload }, { call, put }) {
      const data = yield call(goodsGroupService.createSingle, payload);
      return data;
    },

    *editSingle({ payload }, { call, put }) {
      const data = yield call(goodsGroupService.editSingle, payload);
      return data;
    },
  ...
  reducers: {
    ...
    setSortMove(state, { payload: { item, moveWay } }) {
      if (item.uid) {
        const currentGoodsGroup = state.goodsGroups.find(n => n.id == item.parent_id).children;
