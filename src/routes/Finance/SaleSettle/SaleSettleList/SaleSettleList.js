...
export default class SaleSettleList extends PureComponent {
  ...
  handleGetList = (filter, pages, sorts, mode) => {
    this.props.dispatch({ type: 'saleSettleList/getList',
      payload: {
        ...filter,
        ...pages,
        sorts,
  ...
  handleDeleteSingle = (id, deal) => {
    if (deal != -1) {
      ...
      this.props.dispatch({ type: 'saleSettleList/deleteSingle',
        payload: {
          id,
          deal_payments: deal,
        } }).then(() => {
  ...
  // 筛选
  handleFilter = (value) => {
    this.props.dispatch({ type: 'saleSettleList/setFilterSaleSettleServerData',
      payload: {
        ...value,
  ...
  render() {
    const { saleSettleList: { saleSettleList, saleSettlePagination }, layoutFilter: { saleSettleFilter } } = this.props;
