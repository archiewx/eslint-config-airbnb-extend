...
import styles from './SaleSettleDetail.less';
...
@connect(state => ({
  saleSettleDetail: state.saleSettleDetail,
}))
export default class SaleSettleDetail extends PureComponent {
  // 删除
  handleDeleteSingle = (id, deal) => {
    if (deal != -1) {
      confirmModal.destroy();
      this.props.dispatch({ type: 'saleSettleDetail/deleteSingle',
        payload: {
          id,
          deal_payments: deal,
        } }).then(() => {
        this.props.dispatch(routerRedux.push('/finance/sale-settle'));
      });
    } else {
      this.props.dispatch({ type: 'saleSettleDetail/deleteSingle',
        payload: {
  ...
  // 打印
  handlePrint = (id) => {
    this.props.dispatch({ type: 'saleSettleDetail/printSettle',
      payload: {
        id,
        round: 1,
      } });
  ...
  handleDeletePopConfirm = (payStatus, id) => {
    ...
    if (payStatus == 1) {
      popconfirmModal = (
        <Popconfirm title={<div><span>确认删除此销售结算单?</span></div>} okText="确认" onConfirm={this.handleDeleteSingle.bind(null, id, -1)}>
          删除
        </Popconfirm>
      );
    } else if (payStatus == 3) {
      popconfirmModal = (
        <Popconfirm title={<div><span>确认删除此销售结算单?</span><div style={{ color: 'red' }}>关联的流水也将一同删除</div></div>} okText="继续" onConfirm={this.handleAgianPopConfirm.bind(null, id)}>
          删除
        </Popconfirm>
      );
    }
