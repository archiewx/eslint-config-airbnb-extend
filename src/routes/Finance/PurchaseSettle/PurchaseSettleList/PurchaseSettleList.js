...
export default class PurchaseSettleList extends PureComponent {
  ...
  handleGetList = (filter, pages, sorts, mode) => {
    this.props.dispatch({
      type: 'purchaseSettleList/getList',
      payload: {
        ...filter,
        ...pages,
        sorts,
  ...
  handleDeleteSingle = (id, deal) => {
    if (deal != -1) {
      ...
      this.props
        .dispatch({
          type: 'purchaseSettleList/deleteSingle',
          payload: {
            id,
            deal_payments: deal,
          },
  ...
  handleFilter = (search) => {
    this.props.dispatch({
      type: 'purchaseSettleList/setFilterPurchaseSettleServerData',
      payload: search,
    });
  ...
  render() {
    const {
      purchaseSettleList: { purchaseSettleList, purchaseSettlePagination },
      layoutFilter: { purchaseSettleFilter },
    } = this.props;
    ...
    return (
      <PageHeaderLayout breadcrumbList={breadcrumbList}>
        ...
        <FilterPicker onChange={this.handleFilter} filters={purchaseSettleFilter} />
        <Card bordered={false} title="进货结算列表" extra={tableSortExtra}>
          ...
          <div style={{ marginTop: -43, width: 300 }}>
            <span>{`共 ${purchaseSettlePagination.total || ''} 条结算 第 ${
              pages.page
            } / ${Math.ceil(
              Number(purchaseSettlePagination.total) / Number(pages.per_page),
            )} 页`}</span>
          </div>
        </Card>
      </PageHeaderLayout>
