...
export default class PurchaseSettleList extends PureComponent {
  ...
  handleGetList = (filter, pages, sorts, mode) => {
    this.props.dispatch({ type: 'purchaseSettleList/getList',
      payload: {
        ...filter,
        ...pages,
        sorts,
  ...
  handleDeleteSingle = (id, deal) => {
    if (deal != -1) {
      ...
      this.props.dispatch({ type: 'purchaseSettleList/deleteSingle',
        payload: {
          id,
          deal_payments: deal,
        } }).then(() => {
  ...
  // 筛选
  handleFilter =(value) => {
    this.props.dispatch({ type: 'purchaseSettleList/setFilterPurchaseSettleServerData',
      payload: {
        ...value,
