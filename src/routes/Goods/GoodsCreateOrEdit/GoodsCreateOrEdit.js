...
export default class GoodsCreateOrEdit extends PureComponent {
  ...
  componentDidMount() {
    (async () => {
      this.props.dispatch({
        type: 'goodsCreateOrEdit/setState',
        payload: {
          showData: {},
        },
      });
      if (this.props.history.location.pathname.indexOf('/goods-edit') > -1) {
        ...
        this.props.dispatch({
          type: 'goodsCreateOrEdit/getSingleGoods',
          payload: { id: match[1] },
        });
      }
    })().then(() => {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (showData.prices) {
      if (!!priceGrades.length && !!shops.length) {
        ...
        if (usePricelelvel === 'yes') {
          if (priceModel === '') {
            ...
            shops.forEach((item) => {
              priceGrades.forEach((subItem) => {
                priceTableValue[`${item.id}_${subItem.id}`] = {
                  ...
                  price:
                    (showData.prices[`${item.id}_${subItem.id}`] || {}).price ||
                    showData.prices.price,
                };
              });
            });
            ...
            showData &&
              showData.selectQuantityStep.length &&
          ...
          showData &&
            ...
            showData.selectQuantityStep.forEach((item) => {
              priceTableValue[`${item.id}`] = {
                quantityrange_id: item.id,
                price:
                  (showData.selectQuantityStep[`${item.id}`] || {}).price || showData.prices.price,
              };
            });
        }
      ...
      !!shops.length &&
      !!units.length &&
      !Object.values(this.state.priceTableValue).length
    ) {
      // 使用价格等级
      if (usePricelelvel === 'yes') {
        if (priceModel === '') {
    ...
    if (showData.selectColors && showData.selectSizes && showData.stocks) {
      if (
        !!warehouses.length &&
        (Object.values(this.state.skuStocks).length == 0 ||
          Object.values(this.state.skuStocks).length == warehouses.length ||
          Object.values(this.state.skuStocks).length ==
            warehouses.length * this.state.selectColors.length ||
          Object.values(this.state.skuStocks).length ==
            warehouses.length * this.state.selectColors.length * this.state.selectSizes.length)
      ) {
        const skuStocks = {};
          ...
          warehouses.forEach((item) => {
            showData.selectColors.forEach((colorItem) => {
              skuStocks[`${item.id}_${colorItem.id}`] = {
                warehouse_id: item.id,
                store_quantity: (showData.stocks[`${item.id}_${colorItem.id}`] || {})
                  .store_quantity,
              };
            });
          });
            ...
            showData.selectColors.forEach((colorItem) => {
              showData.selectSizes.forEach((sizeItem) => {
                skuStocks[`${item.id}_${colorItem.id}_${sizeItem.id}`] = {
                  warehouse_id: item.id,
                  store_quantity: (
                    showData.stocks[`${item.id}_${colorItem.id}_${sizeItem.id}`] || {}
                  ).store_quantity,
                };
              });
            });
    ...
    if (showData.selectColors && showData.selectSizes && showData.barcodes) {
      if (
        !this.state.selectColors.length &&
        (Object.values(this.state.skuBarcodes).length == 0 ||
          Object.values(this.state.skuBarcodes).length == 1 ||
          Object.values(this.state.skuBarcodes).length == this.state.selectColors.length ||
          Object.values(this.state.skuBarcodes).length ==
            this.state.selectColors.length * this.state.selectSizes.length)
      ) {
        let skuBarcodes = {};
      ...
      if (!err) {
        ...
        for (const key in skuStocks) {
          if (skuStocks[key].store_quantity) {
            skuStocks[key].store_quantity =
              Number(skuStocks[key].store_quantity) * Number(this.state.selecStockUnitNum);
          }
        }
        value.stock = this.state.skuStocks;
        // 转化数据
        this.props.dispatch({
          type: 'goodsCreateOrEdit/setServerData',
          payload: {
            value,
        ...
        if (this.props.goodsCreateOrEdit.showData.id) {
          this.props
            .dispatch({
              type: 'goodsCreateOrEdit/editSingleGoods',
              payload: {
          ...
          this.props
            .dispatch({
              type: 'goodsCreateOrEdit/createSingleGoods',
              payload: {
                serverData: this.props.goodsCreateOrEdit.serverData,
                imageFile: this.props.goodsCreateOrEdit.imageFile,
              },
  ...
  handleCheckItemRef = (rule, value, callback) => {
    ...
    validateDelay.run(() => {
      if (value) {
        ...
        if (
          this.props.goodsCreateOrEdit.showData.item_ref &&
          this.props.goodsCreateOrEdit.showData.item_ref == value
        ) {
          callback();
          this.setState({
            isNeedIcon: false,
          ...
          this.props
            .dispatch({ type: 'goodsCreateOrEdit/checkItemRef', payload: value })
            .then((result) => {
  ...
  handleGetStandardPrice = (e) => {
    ...
    setTimeout(() => {
      ...
      // 判断标准价的格式
      if (
        typeof standardPrice === 'string' &&
        standardPrice.indexOf('.') > -1 &&
        standardPrice.match(/\./g).length == 1
      ) {
        if (
          standardPrice[standardPrice.length - 1] != '.' &&
          !Number.isInteger(Number(standardPrice[standardPrice.length - 1]))
        ) {
      ...
      if (isAutoInput) {
        for (const key in priceTableValue) {
          ...
          if (this.props.configSetting.priceModel == 'unit') {
            if (
              Number(priceTableValue[key].price) ==
                Number(oldstandardPrice) *
                  Number(
                    this.state.selectUnits.find((n) => n.id == priceTableValue[key].unit_id).number,
                  ) ||
              priceTableValue[key].price == null
            ) {
              priceTableValue[key].price = (
            ...
            priceTableValue[key].price == oldstandardPrice ||
            priceTableValue[key].price == null
          ) {
  ...
  handleSelectQuantityStep = ({ item, key, keyPath }) => {
    ...
    if (this.props.configSetting.priceModel == 'quantityrange') {
      const isOnlySame = current.every((item) => {
        return Object.values(priceTableValue).some(
          (subItem) => subItem.quantityrange_id == item.id,
        );
      });
    ...
    setTimeout(() => {
      ...
      if (this.props.configSetting.priceModel == 'unit') {
        ...
        const current = [].concat(
          form.getFieldValue('unit_select')[form.getFieldValue('unit_select').length - 1],
        );
          ...
          if (!isOnlySame) {
            this.props.priceGrade.priceGrades.forEach((subItem) => {
              priceTableValue[`${current[0]}_${subItem.id}`] = {
                ...
                pricelevel_id: subItem.id,
                price:
                  Number(form.getFieldValue('standard_price')) *
                    Number(arr.find((n) => current[0] == n.id).number) || null,
              };
            });
            form.setFieldsValue({ prices_table: priceTableValue });
          }
  ...
  handleGetColorSelect = (arr, e) => {
    ...
    setTimeout(() => {
      ...
      // 当前选中的颜色
      const current = [].concat(
        form.getFieldValue('color_select')[form.getFieldValue('color_select').length - 1],
      );
  ...
  handleGetSizeSelect = (arr, e) => {
    ...
    setTimeout(() => {
      ...
      // 当前选中的尺码
      const current = [].concat(
        form.getFieldValue('size_select')[form.getFieldValue('size_select').length - 1],
      );
  ...
  handleUnitStockSelect = (value) => {
    ...
    for (const key in stock) {
      if (stock[key].store_quantity) {
        onlyStcok[key].store_quantity =
          Number(stock[key].store_quantity) *
          Number(this.state.selecStockUnitNum) /
          Number(selecStockUnitNum);
        stock[key].store_quantity = Math.floor(
          Number(stock[key].store_quantity) *
            Number(this.state.selecStockUnitNum) /
            Number(selecStockUnitNum),
        );
  ...
  render() {
    ...
    const {
      goodsGroup: { goodsGroups },
      color: { colors },
      size: { sizeLibrarys },
      unit: { units },
      priceGrade: { priceGrades },
      priceQuantityStep: { priceQuantitySteps },
      shop: { shops },
    ...
    const unitStockSelect = (
      <Select
        style={{ width: 200 }}
        defaultValue="库存单位：件 ( x 1 )"
        onChange={this.handleUnitStockSelect}
        type="combobox"
        ...
        {units.map((item) => {
          return (
            <Option
              ...
              value={`库存单位：${item.name} ( x ${item.number} )`}>{`${item.name} ( x ${
              item.number
            } )`}</Option>
          );
        })}
    ...
    const breadcrumbList = [
      ...
      {
        title: this.state.type === 'sale' ? '销售' : '进货',
        href: this.state.type === 'sale' ? '/goods-list/goods-sale' : '/goods-list/goods-shipped',
      },
      {
    ...
    return (
      <PageHeaderLayout
        ...
        <Spin
          size="large"
          spinning={!units.length || !priceGrades.length || !shops.length || !warehouses.length}>
          <Card title="属性" bordered={false} className={styles.bottomCardDivided}>
                  ...
                  <FormItem label="货号" hasFeedback={isNeedIcon}>
                    {getFieldDecorator('item_ref', {
                      ...
                      rules: [
                        ...
                        { validator: this.handleCheckItemRef },
                      ],
                  ...
                  <FormItem label="标准价" hasFeedback={!getFieldValue('standard_price')}>
                    {getFieldDecorator('standard_price', {
                      ...
                      <InputNumber
                        ...
                        precision={2}
                        min={0}
                        onChange={this.handleGetStandardPrice}
                      />,
                    )}
                  </FormItem>
                </Col>
                ...
                <div>
                  <FormItem>
                    {getFieldDecorator('prices_table', {
                      ...
                      <PriceTable
                        priceGrades={priceGrades}
                        shops={shops}
                        selectUnits={selectUnits}
                        selectQuantityStep={selectQuantityStep}
            ...
            <Form layout="horizontal" className={styles.leftLabelTitle}>
              <FormItem label="单位" {...formItemLayout} style={this.checkAttributeItem('unit')}>
                {getFieldDecorator('unit_select', {
                  ...
                  <Select
                    mode="multiple"
                    placeholder="请输入单位"
                    onChange={this.handleGetUnitSelect.bind(null, units)}>
                    {units.map((item) => {
                      return (
                        <Option
                          key={item.id}
                          value={item.id.toString()}
                          disabled={item.default == '1'}>{`${item.name} ( x ${
                          item.number
                        } )`}</Option>
                      );
                    })}
              ...
              <FormItem
                label="颜色"
                {...formItemLayout}
                style={this.checkAttributeItem('skuattributetype_1')}>
                {getFieldDecorator('color_select', {
                  ...
                  <Select
                    mode="multiple"
                    placeholder="请选择颜色"
                    onChange={this.handleGetColorSelect.bind(null, colors)}>
                    {colors.map((item) => {
                      return (
              ...
              <FormItem
                label="尺码"
                {...formItemLayout}
                style={this.checkAttributeItem('skuattributetype_2')}>
                {getFieldDecorator('size_select', {
                  initialValue: showData.sizes || [],
                })(
                  <Select
                    mode="multiple"
                    onChange={this.handleGetSizeSelect.bind(null, sizeLibrarys)}
                    placeholder={selectColors.length == 0 ? '请先选择颜色' : '请选择尺码'}
                    disabled={selectColors.length == 0}>
          ...
          <Card
            title="库存"
            bordered={false}
            className={cx({ bottomCardDivided: true, cardOuterTab: true })}
            extra={unitStockSelect}>
            {warehouses.length === 0 ? (
              <Form>
                <FormItem>
              ...
              <Card
                ...
                onTabChange={this.handleStockTabChange}
                type="inner"
                bordered={false}
                className={styles.cardInnerTab}>
                <Form>
                  <FormItem>
                    {getFieldDecorator('stock', {
                      initialValue: skuStocks,
                ...
                <FormItem>
                  {getFieldDecorator('barcode', {
                    ...
                    <BarCodeTable
                      selectColors={selectColors}
                      selectSizes={selectSizes}
                      itemBarcodeLevel={itemBarcodeLevel}
                    />,
                  )}
                </FormItem>
              </Form>
          ...
          <FooterToolbar>
            <div id="noScroll">
              <Popconfirm
                ...
                title={showData.item_ref ? '确认放弃编辑商品?' : '确认放弃新建商品?'}
