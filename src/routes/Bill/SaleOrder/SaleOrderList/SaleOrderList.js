...
export default class SaleOrderList extends PureComponent {
  ...
  handleDeleteSingle = (id, deal) => {
    if (deal != -1) {
      ...
      this.props
        .dispatch({
          type: 'saleOrderList/deleteSingle',
          payload: {
            id,
            deal_payments: deal,
          },
  ...
  handleFilter = (search) => {
    this.props.dispatch({
      type: 'saleOrderList/setFilterSaleOrderServerData',
      payload: search,
    });
  ...
  render() {
    ...
    const { saleOrderFilter } = layoutFilter;
    const { sorts, pages, filter, sortOrder, sortValue } = this.state;

    const radios = ['settle_way', 'pay_status'];
    ...
    return (
      <PageHeaderLayout breadcrumbList={breadcrumbList}>
        ...
        <FilterPicker onChange={this.handleFilter} filters={saleOrderFilter} radios={radios} />
        <Card bordered={false} title="销售单列表" extra={tableSortExtra}>
          ...
          <div style={{ marginTop: -43, width: 300 }}>
            <span>{`共 ${pagination.total || ''} 条销售单 第 ${pages.page} / ${Math.ceil(
