...
export default class SaleOrderDetail extends PureComponent {
  handleDeleteSingle = (id, deal = -1) => {
    if (deal != -1) {
      ...
      this.props.dispatch({ type: 'saleOrderDetail/deleteSingle',
        payload: {
          id,
          deal_payments: deal,
        } });
  ...
  handlePrint = (id) => {
    this.props.dispatch({ type: 'saleOrderDetail/printSaleOrder',
      payload: {
        id,
        round: 1,
      } });
  ...
  handleExpandedRowRender = (record) => {
    const itemExtraList = this.props.saleOrderDetail.singleOrderDetail.itemExtraList[`${record.item_id}`] || [];
  ...
  render() {
    ...
    const menu = (
      <Menu>
        <Menu.Item key="1">{this.handleDeletePopConfirm(singleOrderDetail.settle_way, singleOrderDetail.pay_status, singleOrderDetail.delivery_status, singleOrderDetail.id)}</Menu.Item>
      </Menu>
    );
    ...
    const action = (
      <div>
        <ButtonGroup>
          <Popconfirm title="确认打印销售单" placement="bottom" onConfirm={this.handlePrint.bind(null, singleOrderDetail.id)}><Button >打印</Button></Popconfirm>
    ...
    const description = (
      <DescriptionList className={styles.headerList} size="small" col="2">
        ...
        <Description term="发货方式">{singleOrderDetail.deliverWay || ''}<span style={{ marginLeft: 10, color: singleOrderDetail.deliverStatus != 3 ? 'red' : 'rgba(0, 0, 0, 0.65)' }}>{singleOrderDetail.deliverStatus || ''}</span></Description>
            ...
            singleOrderDetail.address_id ? (
              <div>
                <Row style={{ marginBottom: 20 }}>
                  <Col span="10">
                    <label className={styles.labelTitle}>收件人：</label><span>{singleOrderDetail.name || ''}</span>
                  </Col>
                  <Col span="10">
                    <label className={styles.labelTitle}>手机号：</label><span>{singleOrderDetail.phone || ''}</span>
                  </Col>
                </Row>
                <Row>
                  <Col span="24">
                    <label className={styles.labelTitle}>收货地址：</label><span>{singleOrderDetail.address || ''}</span>
                  </Col>
                </Row>
                ...
                <Col span="10" >
                  <label className={styles.labelTitle}>调价方式：</label>
                  {
                    singleOrderDetail.adjustWays.map((n, index) => {
                      return index == 0 ? <span key={index}><span>{`${n.name}：`}</span><span>{n.value || ''}</span></span> : <div key={index} style={{ paddingLeft: 70 }}><span>{`${n.name}：`}</span><span>{n.value || ''}</span></div>;
                    })
                  }
                </Col>
              ) : null
