...
export default class DeliverOrderList extends PureComponent {
  ...
  handleFilter = (search) => {
    this.props.dispatch({
      type: 'deliverOrderList/setFilterSaleOrderServerData',
      payload: search,
    });
      ...
      <PageHeaderLayout breadcrumbList={breadcrumbList}>
        ...
        <Card bordered={false} title="调货单列表" extra={tableSortExtra}>
          ...
          <div style={{ marginTop: -43, width: 300 }}>
            <span>{`共 ${deliverOrderPagination.total || ''} 条调货单 第 ${
              pages.page
            } / ${Math.ceil(
              Number(deliverOrderPagination.total) / Number(pages.per_page),
            )} 页`}</span>
          </div>
        </Card>
      </PageHeaderLayout>
