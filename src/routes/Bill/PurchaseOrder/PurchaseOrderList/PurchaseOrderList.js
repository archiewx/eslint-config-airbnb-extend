...
export default class PurchaseOrderList extends PureComponent {
  ...
  handleGetList = (filter, pages, sorts) => {
    this.props.dispatch({ type: 'purchaseOrderList/getList',
      payload: {
        ...filter,
        ...pages,
        sorts,
  ...
  handleDeleteSingle = (id, deal) => {
    if (deal != -1) {
      ...
      this.props.dispatch({ type: 'purchaseOrderList/deleteSingle',
        payload: {
          id,
          deal_payments: deal,
        } }).then(() => {
  ...
  // 筛选
  handleFilter = (value) => {
    this.props.dispatch({ type: 'purchaseOrderList/setFilterSaleOrderServerData',
      payload: {
        ...value,
  ...
  handleMoreOperation = (item) => {
    return (
      <div>
        ...
        <Dropdown overlay={
          <Menu>
            <Menu.Item key="1">{this.handleDeletePopConfirm(item.settle_way, item.pay_status, item.id)}</Menu.Item>
          </Menu>
        }
        >
