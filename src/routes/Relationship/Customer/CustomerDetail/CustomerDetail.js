...
export default class CustomerDetail extends PureComponent {
  ...
  handleChangeCustomerStatus = (id, status) => {
    this.props.dispatch({
      type: 'customerDetail/changeCustomerStatus',
      payload: {
        id,
        freeze: status == 1 ? 0 : 1,
      },
  ...
  handleToCustomerEdit = () => {
    this.props.dispatch(
      routerRedux.push(`/relationship/customer-edit/${this.props.customerDetail.currentId.id}`),
    );
  };
  ...
  handleSaleHistorySort = (pagination, filter, sorter) => {
    ...
    this.props.dispatch({
      type: 'customerDetail/getSaleHistory',
      payload: {
  ...
  handleSaleFilter = (search) => {
    this.props.dispatch({
      type: 'customerDetail/setFilterSaleServerData',
      payload: search,
    });
  ...
  handleGoodsHistorySort = (pagination, filter, sorter) => {
    ...
    this.props.dispatch({
      type: 'customerDetail/getGoodsHistory',
      payload: {
  ...
  // 商品历史筛
  handleGoodsFilter = (search) => {
    this.props.dispatch({
      type: 'customerDetail/setFilterGoodsServerData',
      payload: search,
    });
  ...
  handlePaymentHistorySort = (pagination, filter, sorter) => {
    ...
    this.props.dispatch({
      type: 'customerDetail/getPaymentHistory',
      payload: {
  ...
  // 收银历史筛选你
  handlePaymentFilter = (search) => {
    this.props.dispatch({
      type: 'customerDetail/setFilterPurchaseServerData',
      payload: search,
    });
  ...
  handlePaymentsSort = (pagination, filter, sorter) => {
    ...
    this.props.dispatch({
      type: 'customerDetail/getPayments',
      payload: {
        sorts,
        id: this.props.customerDetail.currentId.id,
      },
  ...
  handleSalesorderSort = (pagination, filter, sorter) => {
    ...
    this.props.dispatch({
      type: 'customerDetail/getSalesorder',
      payload: {
        sorts,
        id: this.props.customerDetail.currentId.id,
      },
  ...
  handleStatementSort = (pagination, filter, sorter) => {
    ...
    this.props.dispatch({
      type: 'customerDetail/getStatement',
      payload: {
        sorts,
        id: this.props.customerDetail.currentId.id,
      },
  ...
  render() {
      ...
      <Menu style={{ width: 109 }}>
        <Menu.Item key="1">
          <Popconfirm
            title="确认删除此客户?"
            placement="bottom"
            onConfirm={this.handleDeleteSingleCustomer.bind(null, currentId.id)}>
    ...
    const action = (
      <div>
        <ButtonGroup>
          <Popconfirm
            title={singleCustomerDetail.freeze == 1 ? '确定解冻此客户?' : '确定冻结此客户?'}
            placement="bottom"
            onConfirm={this.handleChangeCustomerStatus.bind(
              null,
              currentId.id,
              singleCustomerDetail.freeze,
            )}>
            <Button>{singleCustomerDetail.freeze == 1 ? '解冻' : '冻结'}</Button>
          </Popconfirm>
        ...
        <Button type="primary" onClick={this.handleToCustomerEdit}>
          编辑
        </Button>
      </div>
    );
    ...
    const goodsColumns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          <Link
            to={`/relationship/customer-detail/goods-purchase-detail/${currentId.id}/${record.id}/${
              singleCustomerDetail.name
            }`}>
            查看
          </Link>
        ),
    ...
    return (
      <PageHeaderLayout
        title={
          <div>
            <span>姓名：{singleCustomerDetail.name || ''}</span>
            <span style={{ fontSize: 14, marginLeft: 30, color: '#666' }}>
              {singleCustomerDetail.freeze === '1' && '已冻结'}
            </span>
          </div>
        }
        logo={
        ...
        <Card bordered={false} style={{ display: activeTabKey == 'detail' ? 'block' : 'none' }}>
          ...
          {singleCustomerDetail.imageFiles && !!singleCustomerDetail.imageFiles.length ? (
            <div>
          ...
          {singleCustomerDetail.addresses && !!singleCustomerDetail.addresses.length ? (
            <div>
              <Divider style={{ marginBottom: 32 }} />
              <div className={styles.title}>地址</div>
            </div>
          ) : null}
          {singleCustomerDetail.addresses &&
            singleCustomerDetail.addresses.map((item, index) => {
              return (
                <div key={item.id}>
          ...
          <Card bordered={false} className={styles.bottomCardDivided}>
            <Row>
              ...
              <Col span={8}>
                <Info
                  title="未结算销售单"
                  value={singleCustomerFinance.salesorder_unpaid || '0.00'}
                  bordered
                />
              </Col>
              <Col span={8}>
                <Info
                  title="未付款结算单"
                  value={singleCustomerFinance.statement_unpaid || '0.00'}
                />
              </Col>
            </Row>
          </Card>
          <Card bordered={false} tabList={tabListNoTitle} onTabChange={this.handleBlanceTabChange}>
            <div style={{ display: activeBlanceTabKey == 'balance' ? 'block' : 'none' }}>
              <Table
                ...
                dataSource={singleCustomerPayments}
                onChange={this.handlePaymentsSort}
                pagination={pagination}
                rowKey="id"
              />
              <div style={{ marginTop: -43, width: 300 }}>
                <span>{`共 ${singleCustomerPayments.length || ''} 条流水 第 ${
                  pagePayments.page
                } / ${Math.ceil(
                  Number(singleCustomerPayments.length) / Number(pagePayments.per_page),
                )} 页`}</span>
              </div>
            </div>
            <div style={{ display: activeBlanceTabKey == 'sale' ? 'block' : 'none' }}>
              <Table
                ...
                dataSource={singleCustomerSalesorders}
                onChange={this.handleSalesorderSort}
                pagination={pagination}
                rowKey="id"
              />
              <div style={{ marginTop: -43, width: 300 }}>
                <span>{`共 ${singleCustomerSalesorders.length || ''} 条销售单 第 ${
                  pageSalesorder.page
                } / ${Math.ceil(
                  Number(singleCustomerSalesorders.length) / Number(pageSalesorder.per_page),
                )} 页`}</span>
              </div>
            </div>
            <div style={{ display: activeBlanceTabKey == 'settle' ? 'block' : 'none' }}>
              <Table
                ...
                dataSource={singleCustomerStatements}
                onChange={this.handleStatementSort}
                pagination={pagination}
                rowKey="id"
              />
              <div style={{ marginTop: -43, width: 300 }}>
                <span>{`共 ${singleCustomerStatements.length || ''} 条结算单 第 ${
                  pageStatement.page
                } / ${Math.ceil(
                  Number(singleCustomerStatements.length) / Number(pageStatement.per_page),
                )} 页`}</span>
              </div>
            </div>
          </Card>
        ...
        <div style={{ display: activeTabKey == 'sale' ? 'block' : 'none' }}>
          ...
          {/* <FilterDatePick
              ...
              dateLabel="sale"
            /> */}
          <FilterPicker onChange={this.handleSaleFilter} filters={saleHistoryFilter} />
          {/* </Card> */}
          <Card bordered={false}>
            <Table
              columns={saleColumns}
              dataSource={singleCustomerSaleHistory}
              onChange={this.handleSaleHistorySort}
              pagination={pagination}
              rowKey="id"
            />
            <div style={{ marginTop: -43, width: 300 }}>
              <span>{`共 ${singleCustomerSaleHistory.length || ''} 条销售单 第 ${
                pageSaleHistory.page
              } / ${Math.ceil(
                Number(singleCustomerSaleHistory.length) / Number(pageSaleHistory.per_page),
              )} 页`}</span>
            </div>
          </Card>
        </div>
        <div style={{ display: activeTabKey == 'goods' ? 'block' : 'none' }}>
          {/* <Card bordered={false} className={styles.bottomCardDivided}>
            <FilterDatePick
              onChange={this.handleGoodsFilter}
              filterOptions={goodsHistoryFilter}
              tagLabel="goods"
              dateLabel="goods"
            />
          </Card> */}
          <FilterPicker onChange={this.handleGoodsFilter} filters={goodsHistoryFilter} />
          <Card bordered={false}>
            <Table
              columns={goodsColumns}
              dataSource={singleCustomerGoodsHistory}
              onChange={this.handleGoodsHistorySort}
              pagination={pagination}
              rowKey="id"
            />
            <div style={{ marginTop: -43, width: 300 }}>
              <span>{`共 ${singleCustomerGoodsHistory.length || ''} 件商品 第 ${
                pageGoodsHistory.page
              } / ${Math.ceil(
                Number(singleCustomerGoodsHistory.length) / Number(pageGoodsHistory.per_page),
              )} 页`}</span>
            </div>
          </Card>
        </div>
        <div style={{ display: activeTabKey == 'payment' ? 'block' : 'none' }}>
          {/* <Card bordered={false} className={styles.bottomCardDivided}>
            <FilterDatePick
              onChange={this.handlePaymentFilter}
              filterOptions={paymentHistoryFilter}
              tagLabel="payment"
              dateLabel="payment"
            />
          </Card> */}
          <FilterPicker onChange={this.handlePaymentFilter} filters={paymentHistoryFilter} />
          <Card bordered={false}>
            <Table
              columns={paymentColumns}
              dataSource={singleCustomerPaymentHistory}
              onChange={this.handlePaymentHistorySort}
              pagination={pagination}
              rowKey="id"
            />
            <div style={{ marginTop: -43, width: 300 }}>
              <span>{`共 ${singleCustomerPaymentHistory.length || ''} 条流水 第 ${
                pagePaymentHistory.page
              } / ${Math.ceil(
                Number(singleCustomerPaymentHistory.length) / Number(pagePaymentHistory.per_page),
              )} 页`}</span>
            </div>
          </Card>
        </div>
