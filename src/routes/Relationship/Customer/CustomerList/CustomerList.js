...
export default class CustomerList extends PureComponent {
  ...
  handleChangeCustomerStatus = (id, status) => {
    this.props
      .dispatch({
        type: 'customerList/changeCustomerStatus',
        payload: {
          id,
          freeze: status,
        },
  ...
  handleFilter = (search) => {
    this.props.dispatch({
      type: 'customerList/setFilterCustomerServerData',
      payload: search,
    });
            ...
            <Menu>
              {item.freeze == 0 ? (
                <Menu.Item key="1">
                  <Popconfirm
                    title="确认冻结此客户?"
                    onConfirm={this.handleChangeCustomerStatus.bind(null, item.id, 1)}>
                    冻结
                  </Popconfirm>
                </Menu.Item>
              ) : (
                <Menu.Item key="2">
                  <Popconfirm
                    title="确认解除冻结此客户?"
                    onConfirm={this.handleChangeCustomerStatus.bind(null, item.id, 0)}>
                    解除冻结
                  </Popconfirm>
                </Menu.Item>
              )}
              <Menu.Item key="3">
                <Popconfirm
                  title="确认删除此客户?"
                  onConfirm={this.handleDeleteSingleCustomer.bind(null, item.id)}>
                  删除
                </Popconfirm>
              </Menu.Item>
            </Menu>
  ...
  render() {
    const {
      customerList: { customerList, customerPagination },
      layoutFilter: { customerFilter },
    } = this.props;
    const { sorts, pages, filter } = this.state;

    const headerExtra = (
      <Button type="primary" onClick={this.handleToCustomerCreate}>
        新建客户
      </Button>
    );
    ...
    const tableSortExtra = (
      <Select
        ...
        {sortOptions.map((item, index) => {
          return (
            <Option key={index} value={`排序方式: ${item.name}`}>
              {item.name}
            </Option>
          );
        })}
    ...
    return (
      <PageHeaderLayout
        ...
        <FilterPicker onChange={this.handleFilter} filters={customerFilter} />
        <Card bordered={false} title="客户列表" extra={tableSortExtra}>
          <Table rowKey="id" columns={columns} dataSource={customerList} pagination={pagination} />
          <div style={{ marginTop: -43, width: 300 }}>
            <span>{`共 ${customerPagination.total || ''} 位客户 第 ${pages.page} / ${Math.ceil(
              Number(customerPagination.total) / Number(pages.per_page),
            )} 页`}</span>
          </div>
        </Card>
      </PageHeaderLayout>
