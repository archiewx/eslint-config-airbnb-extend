...
export default class CustomerList extends PureComponent {
  ...
  handleChangeCustomerStatus = (id, status) => {
    this.props.dispatch({ type: 'customerList/changeCustomerStatus',
      payload: {
        id,
        freeze: status,
      } }).then(() => {
  ...
  handleGetList = (filter, pages, sorts) => {
    this.props.dispatch({ type: 'customerList/getList',
      payload: {
        ...filter,
        ...pages,
        sorts,
  ...
  // 筛选
  handleFilter = (value) => {
    this.props.dispatch({ type: 'customerList/setFilterCustomerServerData',
      payload: {
        ...value,
