  ...
  render() {
    return (
      <PageHeaderLayout breadcrumbList={breadCrumbList(this.props.history.location.pathname)}>
        <Card bordered={false}>
          <div><span className={styles.spanTitle}>店铺共享客户</span><Switch onClick={this.handleConfirm} checked={!!this.props.configSetting.shopShareCustomer} className={styles.switchPosition} checkedChildren="开" unCheckedChildren="关" /></div>
        </Card>
      </PageHeaderLayout>
    );
  }
